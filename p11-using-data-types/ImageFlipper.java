...
public class ImageFlipper {
    ...
    public static Picture flipH(Picture source) {
        ...
        for (int col = 0; col < width; col++) {
            for (int row = 0; row < height; row++) {
                ...
                target.set(col, height - row - 1, pixel);
            }
        }
    ...
    // does not modify input Picture
    public static Picture flipV(Picture source) {
        // create new Picture object - don't modify source
        int width = source.width();
        int height = source.height();
        Picture target = new Picture(width, height);
        ...
        for (int col = 0; col < width; col++) {
            for (int row = 0; row < height; row++) {
                ...
                target.set(width - col - 1, row, pixel);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        flipped = flipV(original);
        flipped.show();

        // flip horizontal and then vertical
        flipped = flipV(flipH(original));
        flipped.show();
